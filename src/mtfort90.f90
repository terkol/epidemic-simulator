...
! Function igrnd(l,h) now gives the proper distribution:          !
!   used floor() instead of int(). A.Kuronen, 2022                !
!                                                                 !
!-----------------------------------------------------------------!
!
...
contains
  ...
  integer function getseed(info,file)
    ...
    if (openok.and.readok) then
       ...
       if (printinfo) write(0,'(a,a,a,i0)') 'Seed from ',trim(rdev),': ',rn
    else
       call date_and_time(values=t)
       rn=t(7)+60*(t(6)+60*(t(5)+24*(t(3)-1+31*(t(2)-1+12*t(1)))))+t(8)
       if (printinfo) write(0,'(a,i12)') 'Seed from time:',rn
    end if
